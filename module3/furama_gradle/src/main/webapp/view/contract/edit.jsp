<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%--
  Created by IntelliJ IDEA.
  User: ADMIN
  Date: 5/30/2021
  Time: 10:13 AM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Edit customer</title>
    <link rel="stylesheet" href="../../bootstrap4/bootstrap.min.css">
    <style>
        td:first-child {
            width: 24%;
            line-height: 40px;
        }

        td {
            padding: 5px !important;
        }

        input {
            box-shadow: 1px 1px #637fff;
        }

        .fborder {
            box-shadow: 4px 4px #6c757d;
        }
    </style>
</head>
<body>
<div class="container">
    <div class="row mt-4">
        <div class="col-3"></div>
        <div class="col-6 fborder border-secondary rounded">
            <h2 class="text-white rounded bg-info px-5 py-2 mt-2">Edit contract <sup><a href="/contracts"
                                                                                        class="text-primary">x</a></sup>
            </h2>
            <p>
                <span class="text-success">${message}</span>
            </p>
            <form method="post">
                <table class="table table-borderless">
                    <tbody>
                    <tr>
                        <td>Id</td>
                        <td>
                            <lable name="id">${contract.getId()}</lable>
                        </td>
                    </tr>
                    <tr>
                        <td>Start date</td>
                        <td><input class="form-control" type="text" value="${contract.getStartDate()}" name="start">
                        </td>
                    </tr>
                    <tr>
                        <td>End date</td>
                        <td><input class="form-control" type="text" value="${contract.getEndDate()}" name="end">
                        </td>
                    </tr>
                    <tr>
                        <td>Deposit</td>
                        <td><input class="form-control" type="number" value="${contract.getDeposit()}" name="deposit">
                        </td>
                    </tr>
                    <tr>
                        <td>Total Money</td>
                        <td><input class="form-control" type="number" value="${contract.getTotalMoney()}" name="total">
                        </td>
                    </tr>
                    <tr>
                        <td>Employee id</td>
                        <td>
                            <select name="eId" id="">
                                <c:forEach items="${employees}" var="employee">
                                    <option value="${employee.id} ${employee.id == contract.employeeId ? "selected":""} ">
                                            ${employee.getName()}
                                    </option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td>Customer id</td>
                        <td>
                            <select name="cId">
                                <c:forEach items="${customers}" var="customer">
                                    <option value="${customer.id} ${customer.id == contract.customerId ? "selected":""} ">
                                            ${customer.getName()}
                                    </option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td>Service id</td>
                        <td>
                            <select name="sId">
                                <c:forEach items="${services}" var="service">
                                    <option value="${service.id} ${service.id == contract.customerId ? "selected":""} ">
                                            ${service.getName()}
                                    </option>
                                </c:forEach>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <td><a href="/customer-using" class="btn btn-secondary">Close</a></td>
                        <td class="text-right"><input type="submit" value="Edit" class="btn btn-warning"></td>
                    </tr>
                    </tbody>
                </table>
            </form>
        </div>
        <div class="col-3"></div>
    </div>
</div>


<script src="../../bootstrap4/jquery-3.6.0.min.js"></script>
<script src="../../bootstrap4/popper.min.js"></script>
<script src="../../bootstrap4/bootstrap.min.js"></script>
</body>
</html>
